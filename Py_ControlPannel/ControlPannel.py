...
# # BACKEND_TEAM_URL = "http://192.168.100.73:8080/internal/analysis-result"
# BACKEND_TEAM_URL = "http://localhost:5083/internal/sandbox-callback"
...
#             requests.post(BACKEND_TEAM_URL, json=payload, timeout=10)
...
"""
control_panel.py (v9 -- UUID Job Correlation)

Changes from v8:
  - Job ids are now UUIDs (uuid.uuid4()), not an in-memory integer counter.
    This fixes a correlation bug on the .NET side: the old counter could
    theoretically collide or be misread as the .NET ObservationId, since
    both were small sequential integers. A UUID can never be confused with
    a database primary key.
  - /api/files now returns {"jobId": "<uuid>", "status": "Queued"} instead
    of {"observationId": <int>, "status": "Queued"}.
  - The webhook payload now sends "jobId" instead of "observationId".
  - GET /api/observations/<job_id> now takes a string (UUID), not an int.

Everything else (queue, worker, multi-VM selection, sandbox_agent.py
dispatch, telemetry passthrough) is unchanged from v8.
"""
...
# Point this at MalwareSandbox's /internal/sandbox-callback endpoint.
# For local testing this is typically http://localhost:5083/internal/sandbox-callback
BACKEND_TEAM_URL = "http://192.168.100.58:5083/internal/sandbox-callback"
# BACKEND_TEAM_URL = "http://localhost:8080/internal/sandbox-callback"

AGENT_API_KEY = "7f3c9a1e6d4b8c2f0a5e7b9d3c1f6a8e2d4b0c9f7e5a1d6"
AGENT_HEALTH_TIMEOUT = 4
AGENT_ANALYZE_TIMEOUT = 1000
# ============================================================================
...
def _generate_job_id():
    """Every sandbox job gets its own unique UUID. Never reuse, never use
    a sequential counter - the .NET side correlates results by this value
    alone, and a collision here would corrupt someone else's Observation."""
...
def _dispatch_job(job_id, file_bytes, filename, agent_url, vm_id):
    verdict = "Unknown"
    telemetry = {}
    try:
        resp = requests.post(
        ...
        _update_job(job_id, status="Completed", verdict=verdict, features=telemetry)

    except Exception as exc:
        print(f"[!] Job {job_id} failed: {exc}")
        _update_job(job_id, status="Failed", verdict="Failed", features={"error": str(exc)})
        verdict = "Failed"
        ...
        try:
            ...
            print(f"[+] The result for job {job_id} updated in database")
            requests.post(BACKEND_TEAM_URL, json=payload, timeout=10)
        except Exception as e:
            print(f"[!] Warning: Failed to reach webhook: {e}")
...
def _worker():
    while True:
        job_id, file_bytes, filename = _job_queue.get()
        _update_job(job_id, status="Running", features={})
        ...
        threading.Thread(
            target=_dispatch_job,
            args=(job_id, file_bytes, filename, agent_url, target_vm_id),
            daemon=True,
        ).start()
...
def upload_file():
    ...
    _job_queue.put((job_id, file_bytes, uploaded.filename))
...
@app.route("/api/observations/<job_id>", methods=["GET"])
...
if __name__ == "__main__":
    print("[+] Sandbox API Server started (UUID Job Correlation Active).")
    app.run(host="0.0.0.0", port=5005, threaded=True)